...
void jack_shutdown (void *arg)
{
  ...
  const char **ports;
  ports = jack_get_ports (looper.client, NULL, NULL, JackPortIsPhysical|JackPortIsOutput);
  if (ports == NULL) {
...
int main (int argc, char *argv[])
{
  ...
  */
  ...
  ports = jack_get_ports (looper.client, NULL, NULL, JackPortIsPhysical|JackPortIsInput);
  if (ports == NULL) {
    fprintf(stderr, "no physical playback ports\n");
    exit (1);
  }
  ...
  // TODO: Init Looper - pass # tracks, pass # groups?, and stereo/mono
    ...
    // Config Track Manager:
      ...
      // track manager - manage offsets of tracks
    // Init groups - group # and track # collection only?
    // Tra
